...
// creating the Instances for placing the payment into the Razorpay
...
exports.addCartItem = async(req, res) => {
    try {
        ...
        }, { CID, product_id, quantity, DID }, { upsert: true, new: true });
...
exports.calculate = async(req, res) => {
    try {
        ...
        if (order_data.status === 200) {
            ...
            order_data = await order.findOneAndUpdate({ O: order_id }, {...order_data.data }, { upsert: true, new: true });
        } else {
...
async function CreateOrder(CID, DID, items, promo_id, address_id) {
    try {
        ...
        if (promo_id)
            promoData = await coupon.findOne({ _id: promo_id }, {
                coupon_type: 1,
                flat_amount: 1,
                off: 1,
            });
...
exports.CODCheckOut = async(req, res) => {
    try {
        ...
        order_data = await order.findOneAndUpdate({ $and: [{ O: order_id }, query] }, {
    ...
    try {
        ...
        if (payment_ID_check) {
            ...
            if (digest === razorpay_signature) {
                checkOrder = await order.findOneAndUpdate({ O: order_id }, {
                    payment_status: true,
