...
exports.login = async(req, res) => {
    try {
        ...
        let data = await userDB.findOne({ email: email.toLowerCase() }, { _id: 0, username: 1, email: 1, CID: 1, DID: 1, password: 1 });
        if (data) {
...
exports.getCustomerAddress = async(req, res) => {
        try {
            // let query = {DID : req.sessionID}
            let { CID } = req.query;
            let data = CID ? await userDB.findOne({ CID }, {
                address: 1
            }) : await guest.findOne({ DID: req.sessionID }, {
                address: 1
            });
            if (data)
                return res.send({
...
// this func is sending the current Session to the client
