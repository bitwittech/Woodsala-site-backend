            ...
            filterArray.push({
                product_title: {
                    $regex: req.query.product_title.includes(")") ?
                        req.query.product_title.split(")")[1] : req.query.product_title,
                    $options: "i",
                },
            });
...
exports.fetchVariants = async(req, res) => {
    ...
    try {
        ...
        let variants = await product.find({ ACIN: ACIN }, {
            _id: 1,
            SKU: 1,
            primary_material: 1,
            product_title: 1,
...
exports.addReply = async(req, res) => {
    try {
        ...
        let response = await review.findOneAndUpdate({ _id: req.body._id }, { review: reply });
...
exports.verifyCoupon = async(req, res) => {
    ...
    // this controller is used for provide the SKU and some details about the product for autocomplete fields
    ...
    try {
        ...
        if (!search)
            return res.status(203).send({
                status: 203,
                message: "Please provide search hints.",
                data: []
            })

        search = new RegExp(search, "i");

        let list = await product.find({ SKU: search }, {
            _id: 0,
            SKU: 1,
            product_title: 1,
            length_main: 1,
        ...
        console.log(err);
        return res.status(500).send({ status: 500, message: 'Something went wrong.', data: [] });
    }
}

// this controller provide the dropdown for hardware and other custom stuff
...
exports.getMiscellaneous = async(req, res) => {
    try {
        ...
        data = {
            upholstery: await hardware.find({ sub_category_name: "Fabric" }, { _id: 0, title: 1 }),
            material: await material.find({ primaryMaterial_status: true }, { _id: 0, primaryMaterial_name: 1 }),
            polish: await polish.find({}, { _id: 0, polish_name: 1 })
        }
